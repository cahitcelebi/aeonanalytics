...
export const SYSTEM_PROMPT =
  `IMPORTANT: Never generate example, illustrative, or made-up data! Only use the actual data from the function response. If there is no function response, reply: 'Sorry, no data could be retrieved for your request. Please ask a more specific metric question or contact your system administrator.'
...
async function resolveGameId(gameId: string): Promise<string> {
  if (!gameId) return gameId;
  if (!isNaN(Number(gameId))) return String(gameId); // already numeric
  // Try to find by apiKey or name
  const game = await Game.findOne({ where: { apiKey: gameId } }) || await Game.findOne({ where: { name: gameId } });
  if (game) return String(game.id);
...
export const availableFunctions: { [key: string]: Function } = {
  getGameMetrics: async ({ gameId, startDate, endDate }: { gameId: string; startDate?: string; endDate?: string }) => {
  ...
  getRetentionData: async ({ gameId, period }: { gameId: string; period?: string }) => {
    gameId = await resolveGameId(gameId);
  ...
  getRevenueAnalytics: async ({ gameId, startDate, endDate }: { gameId: string; startDate?: string; endDate?: string }) => {
    gameId = await resolveGameId(gameId);
  ...
  getUserSegments: async ({ gameId }: { gameId: string }) => {
    gameId = await resolveGameId(gameId);
  ...
  getPerformanceMetrics: async ({ gameId }: { gameId: string }) => {
    gameId = await resolveGameId(gameId);
...
// Gemini ile sohbet başlatmak için yardımcı fonksiyon
  ...
  try {
    ...
    if (functionCallPart && functionCallPart.functionCall?.name && availableFunctions[functionCallPart.functionCall.name]) {
      ...
      if (args.gameId && typeof args.gameId === 'string' && isNaN(Number(args.gameId))) {
        ...
        if (game) args.gameId = String(game.id);
      }
      ...
      return followup.candidates?.[0]?.content?.parts?.map((p: any) => p.text).join(' ') ?? "";
    }
