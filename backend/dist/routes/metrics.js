...
router.get('/overview/:gameId', async (req, res) => {
    ...
    try {
        ...
        try {
            // DAU (bugün)
            dauResult = await pool.query(`SELECT COUNT(DISTINCT s."playerId") as dau
         FROM sessions s
        ...
        `, [
            ...
            prevDauResult = await pool.query(`SELECT COUNT(DISTINCT s."playerId") as dau
       FROM sessions s
         JOIN devices d ON d."playerId" = s."playerId"
         JOIN games g ON g.id = s."gameId"
         WHERE s."gameId" = $1
        ...
        `, [
            ...
            // WAU (son 7 gün)
            wauResult = await pool.query(`SELECT COUNT(DISTINCT s."playerId") as wau
         FROM sessions s
         JOIN devices d ON d."playerId" = s."playerId"
         JOIN games g ON g.id = s."gameId"
       WHERE s."gameId" = $1
      ...
      `, [
            ...
            prevWauResult = await pool.query(`SELECT COUNT(DISTINCT s."playerId") as wau
       FROM sessions s
         JOIN devices d ON d."playerId" = s."playerId"
         JOIN games g ON g.id = s."gameId"
       WHERE s."gameId" = $1
      ...
      `, [
            ...
            // MAU (son 30 gün)
            mauResult = await pool.query(`SELECT COUNT(DISTINCT s."playerId") as mau
       FROM sessions s
         JOIN devices d ON d."playerId" = s."playerId"
         JOIN games g ON g.id = s."gameId"
       WHERE s."gameId" = $1
      ...
      `, [
            ...
            prevMauResult = await pool.query(`SELECT COUNT(DISTINCT s."playerId") as mau
       FROM sessions s
         JOIN devices d ON d."playerId" = s."playerId"
         JOIN games g ON g.id = s."gameId"
         WHERE s."gameId" = $1
      ...
      `, [
            ...
            avgSessionDurationResult = await pool.query(`SELECT COALESCE(AVG(EXTRACT(EPOCH FROM (s."endTime" - s."startTime"))), 0) as avg_duration
         FROM sessions s
         JOIN devices d ON d."playerId" = s."playerId"
         JOIN games g ON g.id = s."gameId"
         WHERE s."gameId" = $1
           ...
           AND ($8::text IS NULL OR s."playerId" = $8::int)`, [
            ...
            prevAvgSessionDurationResult = await pool.query(`SELECT COALESCE(AVG(EXTRACT(EPOCH FROM (s."endTime" - s."startTime"))), 0) as avg_duration
        FROM sessions s
         JOIN devices d ON d."playerId" = s."playerId"
         JOIN games g ON g.id = s."gameId"
        WHERE s."gameId" = $1
           ...
           AND ($8::text IS NULL OR s."playerId" = $8::int)`, [
            ...
            dailySessionsResult = await pool.query(`SELECT COUNT(*) as session_count
       FROM sessions s
         JOIN devices d ON d."playerId" = s."playerId"
         JOIN games g ON g.id = s."gameId"
         WHERE s."gameId" = $1
           ...
           AND ($8::text IS NULL OR s."playerId" = $8::int)`, [
            ...
            prevDailySessionsResult = await pool.query(`SELECT COUNT(*) as session_count
        FROM sessions s
         JOIN devices d ON d."playerId" = s."playerId"
         JOIN games g ON g.id = s."gameId"
        WHERE s."gameId" = $1
           ...
           AND ($8::text IS NULL OR s."playerId" = $8::int)`, [
            ...
            if (dailySessionsResult.rows[0]?.session_count && wauResult.rows[0]?.wau) {
                const userCount = await pool.query(`SELECT COUNT(DISTINCT s."playerId") as user_count
           FROM sessions s
           JOIN devices d ON d."playerId" = s."playerId"
           JOIN games g ON g.id = s."gameId"
           WHERE s."gameId" = $1
            ...
            const userCountPrev = await pool.query(`SELECT COUNT(DISTINCT s."playerId") as user_count
         FROM sessions s
         JOIN devices d ON d."playerId" = s."playerId"
         JOIN games g ON g.id = s."gameId"
         WHERE s."gameId" = $1
           ...
           AND ($8::text IS NULL OR s."playerId" = $8::int)`, [
            ...
            // New Users (Period)
            const newUsersResult = await pool.query(`SELECT COUNT(*) as new_users
         FROM (
        ...
        `, [
            ...
            const newUsersPrevResult = await pool.query(`SELECT COUNT(*) as new_users
         FROM (
           SELECT s."playerId", MIN(s."startTime") as first_session
           FROM sessions s
           JOIN devices d ON d."playerId" = s."playerId"
        ...
        `, [
            ...
            const cohortResult = await pool.query(`SELECT s."playerId", MIN(s."startTime") as first_session
           FROM sessions s
           JOIN devices d ON d."playerId" = s."playerId"
           JOIN games g ON g.id = s."gameId"
           WHERE s."gameId" = $1
        ...
        GROUP BY s."playerId"
          ...
          `, [
            ...
            for (const user of cohort) {
                ...
                const day1Result = await pool.query(`SELECT 1 FROM sessions s WHERE s."playerId" = $1 AND s."gameId" = $2 AND s."startTime" >= $3 AND s."startTime" <= $4 LIMIT 1`, [playerId, gameId, day1Start.toISOString(), day1End.toISOString()]);
                if ((day1Result?.rowCount || 0) > 0)
                    day1++;
                ...
                const day7Result = await pool.query(`SELECT 1 FROM sessions s WHERE s."playerId" = $1 AND s."gameId" = $2 AND s."startTime" >= $3 AND s."startTime" <= $4 LIMIT 1`, [playerId, gameId, day7Start.toISOString(), day7End.toISOString()]);
                if ((day7Result?.rowCount || 0) > 0)
                    day7++;
                ...
                const day30Result = await pool.query(`SELECT 1 FROM sessions s WHERE s."playerId" = $1 AND s."gameId" = $2 AND s."startTime" >= $3 AND s."startTime" <= $4 LIMIT 1`, [playerId, gameId, day30Start.toISOString(), day30End.toISOString()]);
                if ((day30Result?.rowCount || 0) > 0)
                    day30++;
            }
            ...
            churnRateResult = await pool.query(`WITH user_activity AS (
          SELECT 
            s."playerId",
      ...
      SELECT 
        ...
        `, [
            ...
            const testQuery = await pool.query(`SELECT m.*, d.country, d.platform, g."gameVersion", d."deviceModel"
         FROM monetization m
        ...
        `, [String(gameId), safeStartDate, safeEndDate]);
            ...
            const revenueResult = await pool.query(`SELECT COALESCE(SUM(CAST(m.amount AS DECIMAL(10,2))), 0) as totalrevenue
         FROM monetization m
         JOIN devices d ON d."playerId" = m."playerId" AND d."gameId" = m."gameId"
         JOIN games g ON g.id = m."gameId"
         WHERE m."gameId" = $1
        ...
        `, [
            ...
            const revenuePrevResult = await pool.query(`SELECT COALESCE(SUM(m.amount), 0) as totalRevenue
         FROM monetization m
         JOIN devices d ON d."playerId" = m."playerId" AND d."gameId" = m."gameId"
         JOIN games g ON g.id = m."gameId"
         WHERE m."gameId" = $1
        ...
        `, [
            ...
            const payingUsersResult = await pool.query(`SELECT COUNT(DISTINCT m."playerId") as paying_users
         FROM monetization m
         JOIN devices d ON d."playerId" = m."playerId" AND d."gameId" = m."gameId"
         JOIN games g ON g.id = m."gameId"
         WHERE m."gameId" = $1
        ...
        `, [
            ...
            const payingUsersPrevResult = await pool.query(`SELECT COUNT(DISTINCT m."playerId") as paying_users
         FROM monetization m
         JOIN devices d ON d."playerId" = m."playerId" AND d."gameId" = m."gameId"
         JOIN games g ON g.id = m."gameId"
         WHERE m."gameId" = $1
        ...
        `, [
            ...
            const revenueTrendResult = await pool.query(`SELECT m.timestamp::date as date, SUM(CAST(m.amount AS DECIMAL(10,2))) as revenue
       FROM monetization m
         JOIN devices d ON d."playerId" = m."playerId" AND d."gameId" = m."gameId"
         JOIN games g ON g.id = m."gameId"
         WHERE m."gameId" = $1
        ...
        `, [
            ...
            const dauTrendResult = await pool.query(`SELECT s."startTime"::date as date, COUNT(DISTINCT s."playerId") as dau
         FROM sessions s
         JOIN devices d ON d."playerId" = s."playerId"
         JOIN games g ON g.id = s."gameId"
         WHERE s."gameId" = $1
...
router.get('/engagement/:gameId', async (req, res) => {
    try {
        const { gameId } = req.params;
        ...
        const avgSessionDurationResult = await pool.query(`SELECT COALESCE(AVG(EXTRACT(EPOCH FROM (s."endTime" - s."startTime"))), 0) as avg_duration
       FROM sessions s
       JOIN devices d ON d."playerId" = s."playerId"
       JOIN games g ON g.id = s."gameId"
       WHERE s."gameId" = $1
      ...
      `, filterParams);
        ...
        // Session Count (total)
        const sessionCountResult = await pool.query(`SELECT COUNT(*) as session_count
       FROM sessions s
       JOIN devices d ON d."playerId" = s."playerId"
       JOIN games g ON g.id = s."gameId"
       WHERE s."gameId" = $1
      ...
      `, filterParams);
        ...
        // Active Users (total)
        const activeUsersResult = await pool.query(`SELECT COUNT(DISTINCT s."playerId") as active_users
       FROM sessions s
       JOIN devices d ON d."playerId" = s."playerId"
       JOIN games g ON g.id = s."gameId"
       WHERE s."gameId" = $1
      ...
      `, filterParams);
        ...
        // Returning Users
        const returningUsersResult = await pool.query(`SELECT COUNT(DISTINCT s."playerId") as returning_users
       FROM sessions s
       JOIN devices d ON d."playerId" = s."playerId"
       JOIN games g ON g.id = s."gameId"
       WHERE s."gameId" = $1
      ...
      `, filterParams);
        ...
        const churnRateResult = await pool.query(`WITH user_activity AS (
        SELECT 
          s."playerId",
          DATE_TRUNC('day', s."startTime") as activity_date
        FROM sessions s
      ...
      `, filterParams);
        ...
        const sessionCountByDayResult = await pool.query(`SELECT s."startTime"::date as date, COUNT(*) as session_count
       FROM sessions s
       JOIN devices d ON d."playerId" = s."playerId"
       JOIN games g ON g.id = s."gameId"
       WHERE s."gameId" = $1
      ...
      `, filterParams);
        ...
        // Active Users by Day
        const activeUsersByDayResult = await pool.query(`SELECT s."startTime"::date as date, COUNT(DISTINCT s."playerId") as active_users
       FROM sessions s
       JOIN devices d ON d."playerId" = s."playerId"
       JOIN games g ON g.id = s."gameId"
       WHERE s."gameId" = $1
      ...
      `, filterParams);
        ...
        for (let day = 0; day <= 30; day++) {
            ...
            for (const user of cohort) {
                ...
                const result = await pool.query(`SELECT 1 FROM sessions s WHERE s."playerId" = $1 AND s."gameId" = $2 AND s."startTime" >= $3 AND s."startTime" < $4 LIMIT 1`, [playerId, gameId, dayStart.toISOString(), dayEnd.toISOString()]);
        ...
        const avgSessionDurationPrevResult = await pool.query(`SELECT COALESCE(AVG(EXTRACT(EPOCH FROM (s."endTime" - s."startTime"))), 0) as avg_duration
       FROM sessions s
       JOIN devices d ON d."playerId" = s."playerId"
       JOIN games g ON g.id = s."gameId"
       WHERE s."gameId" = $1
      ...
      `, [
        ...
        const churnRatePrevResult = await pool.query(`WITH user_activity AS (
        SELECT 
          s."playerId",
          DATE_TRUNC('day', s."startTime") as activity_date
        FROM sessions s
...
router.get('/realtime/:gameId', async (req, res) => {
    ...
    try {
        ...
        const activeUsersResult = await pool.query(`SELECT to_char(date_trunc('hour', s."startTime"), 'YYYY-MM-DD HH24:00') as hour,
              COUNT(DISTINCT s."playerId") as active_users
       FROM sessions s
       WHERE s."gameId" = $1
         AND s."startTime" >= $2 AND s."startTime" <= $3
       GROUP BY hour
       ORDER BY hour ASC`, [gameId, startISO, endISO]);
        // New Users per hour (first session in last 24h)
        const newUsersResult = await pool.query(`SELECT to_char(date_trunc('hour', first_session), 'YYYY-MM-DD HH24:00') as hour,
              COUNT(*) as new_users
       FROM (
         SELECT s."playerId", MIN(s."startTime") as first_session
         FROM sessions s
       ...
       ORDER BY hour ASC`, [gameId, startISO, endISO]);
        ...
        const returningUsersResult = await pool.query(`SELECT to_char(date_trunc('hour', s."startTime"), 'YYYY-MM-DD HH24:00') as hour,
       ...
       ORDER BY hour ASC`, [gameId, startISO, endISO]);
        // Revenue per hour
        const revenueResult = await pool.query(`SELECT to_char(date_trunc('hour', m.timestamp), 'YYYY-MM-DD HH24:00') as hour,
       ...
       ORDER BY hour ASC`, [gameId, startISO, endISO]);
        ...
        const transactionsResult = await pool.query(`SELECT to_char(date_trunc('hour', m.timestamp), 'YYYY-MM-DD HH24:00') as hour,
       ...
       ORDER BY hour ASC`, [gameId, startISO, endISO]);
        ...
        const sessionCountResult = await pool.query(`SELECT to_char(date_trunc('hour', s."startTime"), 'YYYY-MM-DD HH24:00') as hour,
       ...
       ORDER BY hour ASC`, [gameId, startISO, endISO]);
        ...
        function mapByHour(result, key) {
            const map = {};
            for (const row of result.rows)
                map[row.hour] = Number(row[key]);
...
router.get('/monetization/:gameId', async (req, res) => {
    ...
    try {
      ...
      `, [
        ...
        // Satın alma sayısı
        const purchaseCountResult = await pool.query(`SELECT COUNT(*) as purchase_count
       FROM monetization m
       JOIN devices d ON d."playerId" = m."playerId" AND d."gameId" = m."gameId"
       JOIN games g ON g.id = m."gameId"
       WHERE m."gameId" = $1
      ...
      `, [
        ...
        const mauResult = await pool.query(`SELECT COUNT(DISTINCT s."playerId") as mau
      FROM sessions s
      JOIN devices d ON d."playerId" = s."playerId"
       JOIN games g ON g.id = s."gameId"
       WHERE s."gameId" = $1
      ...
      `, [
        ...
        const revenueChartResult = await pool.query(`SELECT m.timestamp::date as date, SUM(CAST(m.amount AS DECIMAL(10,2))) as revenue
       FROM monetization m
       JOIN devices d ON d."playerId" = m."playerId" AND d."gameId" = m."gameId"
       JOIN games g ON g.id = m."gameId"
       WHERE m."gameId" = $1
      ...
      `, [
        ...
        const productBreakdownResult = await pool.query(`SELECT m."productId" as product_id, SUM(CAST(m.amount AS DECIMAL(10,2))) as revenue, COUNT(*) as count
       FROM monetization m
       JOIN devices d ON d."playerId" = m."playerId" AND d."gameId" = m."gameId"
       JOIN games g ON g.id = m."gameId"
       WHERE m."gameId" = $1
...
router.get('/monetization/:gameId/paying-user-trend', async (req, res) => {
    ...
    const playerIdParam = !playerId || playerId === 'all' ? null : playerId;
    try {
        const result = await pool.query(`SELECT m.timestamp::date as date, COUNT(DISTINCT m."playerId") as paying_users
       FROM monetization m
       JOIN devices d ON d."playerId" = m."playerId" AND d."gameId" = m."gameId"
       JOIN games g ON g.id = m."gameId"
       WHERE m."gameId" = $1
...
router.get('/user-analysis/:gameId', async (req, res) => {
    ...
    try {
        // 1. Demographics
        const countryDistribution = await pool.query(`SELECT d.country, COUNT(DISTINCT s."playerId") as user_count
       FROM sessions s
       JOIN devices d ON d."playerId" = s."playerId"
       WHERE s."gameId" = $1
         AND s."startTime" >= $2::date AND s."startTime" < $3::date
       ...
       GROUP BY d.country
       ORDER BY user_count DESC`, [gameId, startDate, safeEndDate, countryParam, platformParam, deviceParam, playerIdParam]);
        const platformDistribution = await pool.query(`SELECT d.platform, COUNT(DISTINCT s."playerId") as user_count
       FROM sessions s
       JOIN devices d ON d."playerId" = s."playerId"
       WHERE s."gameId" = $1
         AND s."startTime" >= $2::date AND s."startTime" < $3::date
       ...
       GROUP BY d.platform
       ORDER BY user_count DESC`, [gameId, startDate, safeEndDate, countryParam, platformParam, deviceParam, playerIdParam]);
        const deviceDistribution = await pool.query(`SELECT d."deviceModel", COUNT(DISTINCT s."playerId") as user_count
       FROM sessions s
       JOIN devices d ON d."playerId" = s."playerId"
       WHERE s."gameId" = $1
         AND s."startTime" >= $2::date AND s."startTime" < $3::date
       ...
       ORDER BY user_count DESC`, [gameId, startDate, safeEndDate, countryParam, platformParam, deviceParam, playerIdParam]);
        // 2. User Behavior
        const avgSessionDuration = await pool.query(`SELECT COALESCE(AVG(EXTRACT(EPOCH FROM (s."endTime" - s."startTime"))), 0) as avg_duration
       FROM sessions s
       JOIN devices d ON d."playerId" = s."playerId"
       WHERE s."gameId" = $1
         AND s."startTime" >= $2::date AND s."startTime" < $3::date
         ...
         AND ($5::text IS NULL OR d.platform = $5::text)
         AND ($6::text IS NULL OR d."deviceModel" = $6::text)
         AND ($7::text IS NULL OR s."playerId" = $7::int)`, [gameId, startDate, safeEndDate, countryParam, platformParam, deviceParam, playerIdParam]);
        const sessionsPerUser = await pool.query(`SELECT s."playerId", COUNT(*) as session_count
       FROM sessions s
       JOIN devices d ON d."playerId" = s."playerId"
       WHERE s."gameId" = $1
         AND s."startTime" >= $2::date AND s."startTime" < $3::date
         ...
         AND ($6::text IS NULL OR d."deviceModel" = $6::text)
         AND ($7::text IS NULL OR s."playerId" = $7::int)
       GROUP BY s."playerId"`, [gameId, startDate, safeEndDate, countryParam, platformParam, deviceParam, playerIdParam]);
        // 3. User Segmentation
        const newUsers = await pool.query(`SELECT COUNT(DISTINCT s."playerId") as new_users
       FROM sessions s
       JOIN devices d ON d."playerId" = s."playerId"
       WHERE s."gameId" = $1
         AND s."startTime" >= $2::date AND s."startTime" < $3::date
        ...
        const payingUsers = await pool.query(`SELECT COUNT(DISTINCT m."playerId") as paying_users
       FROM monetization m
       ...
       WHERE m."gameId" = $1
         ...
         AND ($5::text IS NULL OR d.platform = $5::text)
         AND ($6::text IS NULL OR d."deviceModel" = $6::text)
         AND ($7::text IS NULL OR m."playerId" = $7::int)`, [gameId, startDate, safeEndDate, countryParam, platformParam, deviceParam, playerIdParam]);
        // 4. User Lifecycle
        const userLifecycle = await pool.query(`WITH user_metrics AS (
        SELECT
          s."playerId",
      ...
      SELECT
       ...
       FROM user_metrics`, [gameId, startDate, safeEndDate, countryParam, platformParam, deviceParam, playerIdParam]);
        ...
        const userLifecyclePrev = await pool.query(`WITH user_metrics AS (
      SELECT 
        s."playerId",
           MIN(s."startTime") as first_session,
           MAX(s."startTime") as last_session,
      ...
      WHERE s."gameId" = $1
       ...
       SELECT 
         ...
         AVG(total_purchases) as avg_purchases,
         AVG(total_spent) as avg_spent
       FROM user_metrics`, [gameId, prevStartDate, safePrevEndDate, countryParam, platformParam, deviceParam, playerIdParam]);
        // 2. New Users (prev)
        const newUsersPrev = await pool.query(`SELECT COUNT(DISTINCT s."playerId") as new_users
       FROM sessions s
       JOIN devices d ON d."playerId" = s."playerId"
       WHERE s."gameId" = $1
         AND s."startTime" >= $2::date AND s."startTime" < $3::date
        ...
        // 3. Paying Users (prev)
        const payingUsersPrev = await pool.query(`SELECT COUNT(DISTINCT m."playerId") as paying_users
       FROM monetization m
       JOIN devices d ON d."playerId" = m."playerId"
       WHERE m."gameId" = $1
         AND m.timestamp::date >= $2::date AND m.timestamp::date < $3::date
         ...
         AND ($5::text IS NULL OR d.platform = $5::text)
         AND ($6::text IS NULL OR d."deviceModel" = $6::text)
         AND ($7::text IS NULL OR m."playerId" = $7::int)`, [gameId, prevStartDate, safePrevEndDate, countryParam, platformParam, deviceParam, playerIdParam]);
        // 4. Avg. Session Duration (prev)
        const avgSessionDurationPrev = await pool.query(`SELECT COALESCE(AVG(EXTRACT(EPOCH FROM (s."endTime" - s."startTime"))), 0) as avg_duration
       FROM sessions s
       JOIN devices d ON d."playerId" = s."playerId"
       WHERE s."gameId" = $1
         AND s."startTime" >= $2::date AND s."startTime" < $3::date
         ...
         AND ($5::text IS NULL OR d.platform = $5::text)
         AND ($6::text IS NULL OR d."deviceModel" = $6::text)
         AND ($7::text IS NULL OR s."playerId" = $7::int)`, [gameId, prevStartDate, safePrevEndDate, countryParam, platformParam, deviceParam, playerIdParam]);
        // 5. Avg. Sessions/User (prev)
        const sessionsPerUserPrev = await pool.query(`SELECT s."playerId", COUNT(*) as session_count
       FROM sessions s
       JOIN devices d ON d."playerId" = s."playerId"
       WHERE s."gameId" = $1
         AND s."startTime" >= $2::date AND s."startTime" < $3::date
         ...
         AND ($6::text IS NULL OR d."deviceModel" = $6::text)
         AND ($7::text IS NULL OR s."playerId" = $7::int)
       GROUP BY s."playerId"`, [gameId, prevStartDate, safePrevEndDate, countryParam, platformParam, deviceParam, playerIdParam]);
...
router.get('/performance/:gameId', async (req, res) => {
    try {
        ...
        const currentData = await getPerformanceData(String(gameId), typeof startDate === 'string' ? startDate : '', typeof endDate === 'string' ? endDate : '', typeof country === 'string' ? country : undefined, typeof version === 'string' ? version : undefined, typeof platform === 'string' ? platform : undefined, typeof device === 'string' ? device : undefined, typeof playerId === 'string' ? playerId : undefined);
        ...
        const prevData = await getPerformanceData(String(gameId), prevStart.toISOString().split('T')[0], prevEnd.toISOString().split('T')[0], typeof country === 'string' ? country : undefined, typeof version === 'string' ? version : undefined, typeof platform === 'string' ? platform : undefined, typeof device === 'string' ? device : undefined, typeof playerId === 'string' ? playerId : undefined);
        // Calculate rates with proper handling
        const calculateRate = (current, total) => {
            if (!total)
                return 0;
...
async function getPerformanceData(gameId, startDate, endDate, country, version, platform, device, playerId) {
...
router.get('/events/:gameId', async (req, res) => {
    ...
    try {
        ...
        const totalEventsResult = await pool.query(`SELECT COUNT(*) as total FROM events e
       JOIN sessions s ON e."sessionId" = s.id
       JOIN devices d ON d."playerId" = s."playerId" AND d."gameId" = s."gameId"
       WHERE s."gameId" = $1
      ...
      `, [gameId, safeStartDate, safeEndDate, eventTypeParam, eventNameParam, platformParam, deviceParam, countryParam, versionParam, playerIdParam]);
        ...
        const totalEventsPrevResult = await pool.query(`SELECT COUNT(*) as total FROM events e
       JOIN sessions s ON e."sessionId" = s.id
       JOIN devices d ON d."playerId" = s."playerId" AND d."gameId" = s."gameId"
       WHERE s."gameId" = $1
         AND e.timestamp >= $2::date AND e.timestamp < $3::date
      ...
      `, [gameId, prevStartDate, prevEndDate, eventTypeParam, eventNameParam, platformParam, deviceParam, countryParam, versionParam, playerIdParam]);
        ...
        const typeDistResult = await pool.query(`SELECT e."eventType", COUNT(*) as count
       FROM events e
       JOIN sessions s ON e."sessionId" = s.id
       JOIN devices d ON d."playerId" = s."playerId" AND d."gameId" = s."gameId"
       WHERE s."gameId" = $1
      ...
      `, [gameId, safeStartDate, safeEndDate, eventTypeParam, eventNameParam, platformParam, deviceParam, countryParam, versionParam, playerIdParam]);
        // Zaman serisi (trend)
        const trendResult = await pool.query(`SELECT e.timestamp::date as date, COUNT(*) as count
       FROM events e
       JOIN sessions s ON e."sessionId" = s.id
       JOIN devices d ON d."playerId" = s."playerId" AND d."gameId" = s."gameId"
       WHERE s."gameId" = $1
      ...
      `, [gameId, safeStartDate, safeEndDate, eventTypeParam, eventNameParam, platformParam, deviceParam, countryParam, versionParam, playerIdParam]);
        ...
        const topEventsResult = await pool.query(`SELECT e."eventName", COUNT(*) as count
       FROM events e
       JOIN sessions s ON e."sessionId" = s.id
       JOIN devices d ON d."playerId" = s."playerId" AND d."gameId" = s."gameId"
       WHERE s."gameId" = $1
      ...
      `, [gameId, safeStartDate, safeEndDate, eventTypeParam, eventNameParam, platformParam, deviceParam, countryParam, versionParam, playerIdParam]);
        // Platform breakdown
        const platformResult = await pool.query(`SELECT d.platform, COUNT(*) as count
       FROM events e
       JOIN sessions s ON e."sessionId" = s.id
       JOIN devices d ON d."playerId" = s."playerId" AND d."gameId" = s."gameId"
       WHERE s."gameId" = $1
      ...
      `, [gameId, safeStartDate, safeEndDate, eventTypeParam, eventNameParam, platformParam, deviceParam, countryParam, versionParam, playerIdParam]);
        ...
        const countryResult = await pool.query(`SELECT d.country, COUNT(*) as count
       FROM events e
       JOIN sessions s ON e."sessionId" = s.id
       JOIN devices d ON d."playerId" = s."playerId" AND d."gameId" = s."gameId"
       WHERE s."gameId" = $1
