...
router.get('/:id/metrics', async (req, res) => {
    ...
    `;
        ...
        const activityChart = dailyDau.map(row => ({
            date: row.day.toISOString().split('T')[0],
            users: parseInt(row.dau)
        }));
        // Event chart: daily events
        const eventChart = dailyEvents.map(row => ({
            date: row.day.toISOString().split('T')[0],
            events: parseInt(row.events)
        }));
        // New user chart
        const newUserChart = dailyNewUsers.map(row => ({
            date: row.day.toISOString().split('T')[0],
            newUsers: parseInt(row.newUsers)
        }));
        // Session chart
        const sessionChart = dailySessions.map(row => ({
            date: row.day.toISOString().split('T')[0],
        ...
        const avgSessionDuration = avgSession && avgSession['avgDuration'] ? Math.round(Number(avgSession['avgDuration'])) : 0;
    ...
    `;
        ...
        res.json({
            dau: parseInt(dauResult[0].dau) || 0,
            wau: parseInt(wauResult[0].wau) || 0,
            mau: parseInt(mauResult[0].mau) || 0,
            avgSessionDuration,
            retention,
            activityChart,
